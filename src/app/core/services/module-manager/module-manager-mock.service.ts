    ...
    return {
      ...
      configs: {
        greeting: {
          ...
          opt_ext: false,
          type: 'text',
          type_opt: {max_len: {value: 20, data_type: 'int'}},
          data_type: 'string',
          is_slice: false,
          required: true,
        },
        port: {
          ...
          opt_ext: false,
          type: 'number',
          type_opt: {min: {value: 1024, data_type: 'int'}, max: {value: 65535, data_type: 'int'}},
          data_type: 'int',
          is_slice: false,
          required: true,
        },
  ...
  loadRepositoryModules(filter?: RepoModulesFilter): Observable<RepoModule[]> {
    ...
    if (filter?.repositories && filter.repositories.length > 0) {
      modules = modules.filter((m) =>
        (m.repository_variants || []).some((v) => filter.repositories!.includes(v.source)),
      );
    }
